...
<html lang="en">
    ...
    <body>
        ...
        <!-- Blog Header Section -->
        <div class="blog-header bg-dark text-white py-2" style="margin-bottom: 40px;">
            <div class="container">
                <h1 class="text-center mb-0" style="font-size: 4.5rem;">Blog</h1>
                <p class="text-center" style="font-size: 1.7rem; margin-top: 5px;">
                    <a href="index.php" style="color: #fff; text-decoration: none;">Home</a> / <span style="font-weight: bold; color: #fff;">Blog</span>
                </p>
            </div>
        </div>
        ...
        <div class="container py-5">
            <div class="row">
                <div class="col-lg-8">
                    ...
                    $blog_post_products_query = mysqli_query($con, "SELECT p.id, p.productName, p.productImage1, p.postingDate, c.categoryName FROM products p JOIN category c ON p.category = c.id ORDER BY p.id DESC LIMIT $limit OFFSET $offset");
                    while($product = mysqli_fetch_array($blog_post_products_query)) {
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <ul class="list-unstyled mb-0">
                                        <?php
                                        $mainCategoriesSql = mysqli_query($con, "SELECT id, categoryName FROM category");
                                        while($mainCat = mysqli_fetch_array($mainCategoriesSql))
                                        {
                                        ?>
                                            <li>
                                                <a href="shop-categories.php?cid=<?php echo htmlentities($mainCat['id']);?>"><strong><?php echo htmlentities($mainCat['categoryName']);?></strong></a>:
                                                <?php
                                                $subCategories = [];
                                                $subCategoriesSql = mysqli_query($con, "SELECT id, subcategory FROM subcategory WHERE categoryId = '" . $mainCat['id'] . "'");
                                                while($subCat = mysqli_fetch_array($subCategoriesSql)) {
                                                    $subCategories[] = '<a href="category.php?cid=' . htmlentities($mainCat['id']) . '&subid=' . htmlentities($subCat['id']) . '">' . htmlentities($subCat['subcategory']) . '</a>';
                                                }
