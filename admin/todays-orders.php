...
<head>
	...
	<title>FootPrint | Today's Orders</title>
	...
	<div id="layoutSidenav_content">
		<main>
			<div class="container-fluid px-4">
				<h1 class="mt-4 mb-3">Today's Orders</h1>
...
$t1="23:59:59";
$to=date('Y-m-d')." ".$t1;
// Only select today's orders
$query=mysqli_query($con,"select users.name as username,users.email as useremail,users.contactno as usercontact,products.productName as productname,orders.orderDate as orderdate,orders.id as id, orders.orderNumber as orderNumber from orders left join users on orders.userId=users.id left join products on products.id=orders.productId where orders.orderDate between '$from' and '$to'");
$cnt=1;
$hasOrders = false;
...
?>
									<tr>
										...
										<td>
											...
											<a href="edit-order.php?orderNumber=<?php echo urlencode($row['orderNumber']); ?>" title="Edit Order" class="btn btn-warning btn-sm ms-1"><i class="fas fa-edit"></i> Edit</a>
										</td>
									</tr>
<?php $cnt++; } ?>
<?php if(!$hasOrders) { ?>
									<tr>
										<td colspan="8" class="text-center">No orders placed today</td>
									</tr>
<?php } ?>
								</tbody>
							</table>
